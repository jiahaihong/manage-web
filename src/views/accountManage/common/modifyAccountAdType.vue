 ...
 * @LastEditTime: 2022-08-10 11:29:17
...
export default {
  ...
  data() {
    return {
      downloadType: 'UPDATE_AD_TYPE', // 页面标记
      isLoading: false,
      visible: false,
  ...
  methods: {
    ...
    downloadExcelTemplate() {
      downloadExcelTemplate({ type: this.downloadType }).then((res) => {
        if (res.code === 200) {
          this.$ExportXLSX(res.data || [], '更新账号广告类型上传模板.xlsx')
        }
      });
    },
    // 导出数据
    ...
    async SubmitForm() {
      try {
        ...
        this.isLoading = true
        await modifyAccountAdType(params)
    ...
    async getList(page = 1) {
      try {
        ...
        const res = await getModifyAdTypeLog(params)
