 ...
 * @LastEditTime: 2022-08-10 11:36:56
 * @LastEditors: Please set LastEditors
 * @Description: 更新账号bc
-->
<template>
  <div class="modifyAccountBc">
    <el-dialog
  ...
  methods: {
    ...
    downloadExcelTemplate() {
      downloadExcelTemplate({ type: this.downloadType }).then((res) => {
        if (res.code === 200) {
          this.$ExportXLSX(res.data || [], '更新账号bc上传模板.xlsx')
        }
      });
    },
    // 导出数据
    ...
    async getList(page = 1) {
      try {
        ...
        const res = await getModifyBcLog(params)
