          ...
          <div class="global-module table-paddings">
            ...
            >
              ...
              <el-form-item
                label="季度"
                prop="quarter"
                :rules="$setRules({ message: '请选择季度'})"
              >
                <!-- <DateQuter
                  ref="dateQuter"
                  v-model="divisionForm.quarter"
                /> -->
              </el-form-item>
              ...
              <el-form-item label="数据分析">
                <el-row>
                  <el-col :span="6">
                    parent ID总数: {{ exportParams.dataSize }}
                  </el-col>
                  <el-col :span="6">
                    系统中未找到的parent ID: {{ exportParams.notFindParentSize }}
                  </el-col>
                  <el-col :span="2">
                    <el-button
                      type="primary"
              ...
              <el-form-item
                label="数据处理"
                prop="channelId"
                :rules="$setRules({ message: '请选择数据处理！'})"
              >
                <el-checkbox v-model="divisionForm.overwriteFlag">
                  自动将未更新到Parent ID激活类型设置为【非激活客户】，新客类型设置为【非新客】
                </el-checkbox>
              </el-form-item>
            </el-form>
            <div
...
import {
  importParentType,
  exportParentAnalyseResult,
  updateParentType,
  getUpdateParentTypeLog,
} from "@/api/account";
  ...
  methods: {
    ...
    async handleUploadChange(file) {
      try {
        ...
        const res = await importParentType(form);

        const {
          fileName, fileUrl, notFindParentSize, dataSize, exportResultKey, data, errorMsgs
        } = res.data
        ...
        this.exportParams = {
          notFindParentSize, dataSize, exportResultKey
        }
    ...
    async SubmitForm() {
      try {
        ...
        const res = await updateParentType({
          ...last,
    ...
    downloadExcel() {
      downloadExcelTemplate({ type: "UPDATE_PARENT_TYPE" }).then((res) => {
        if (res.code === 200) {
          this.$ExportXLSX(res.data || [], '更新ParentID模板.xlsx')
        }
      });
    },
    exportNotFoundAccountXlsx() {
      if (this.exportParams.notFindParentSize == 0) {
        this.$message({
          message: "暂无可导出数据",
          type: "error",
        });
      ...
      exportParentAnalyseResult({ key: this.exportParams.exportResultKey }).then((file) => {
        this.$CreatDownload({
          file: file,
    ...
    async getList(page = 1) {
      ...
      try {
        ...
        const res = await getUpdateParentTypeLog(params)
